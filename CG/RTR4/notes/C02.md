...
这一阶段的目标是找到三角形内部的所有像素。这个阶段称为光栅化 rasterization，分成两个子阶段：三角形设置 triangle setup（也叫图元装配 primitive assembly）和三角形遍历 triangle traversal。
...
像素处理阶段分成两个子阶段：像素着色 pixel shading 和混合 merging。
...
每个像素的信息存储在颜色缓冲 color buffer 中，它是一个关于颜色的长方形数组（每个颜色有红绿蓝三个成分）。混合阶段负责将片元颜色和缓冲中当前颜色混合，称为 raster operations pipeline ROP 或者 render output unit。
